(function($) {
	var ViewModel = function(options) {
		var self = this;
		this.url = 'https://census.daybreakgames.com/get/ps2/outfit_member?c:limit=1000&c:resolve=online_status,character(name,battle_rank,profile_id)&c:join=type:profile^list:0^inject_at:profile^show:name.en^on:character.profile_id^to:profile_id&outfit_id=' + options.outfitid;
		this.Members = ko.observableArray([]);
		this.HideOffline = ko.observable(true);
		this.IsLoading = ko.observable(false);
		
		this.MembersSortedDate = ko.computed(function(){
			...
			var result = ko.utils.arrayFilter(self.Members(), function(item){
				return item.member_since_date < dStr;
			});
			
			result.sort(function(l,r){ 
			    return ((l.member_since > r.member_since) ? -1 : ((l.member_since < r.member_since) ? 1 : 0));
			});
